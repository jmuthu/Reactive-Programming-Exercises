...
object Main {
  ...
  def main(args: Array[String]) {
    ...
    val myServerSubscription = myServer.start("/test")(req => req.keysIterator)

    // TO IMPLEMENT
    // 2. create a future that expects some user input `x`
    //    and continues with a `"You entered... " + x` message
    val userInterrupted: Future[String] = Future.userInput("Enter text : ").continueWith { "You entered..." + _.now }
    ...
    val terminationRequested: Future[String] = {
      val list = List(userInterrupted, timeOut)
      Future.any[String](list)
    }
